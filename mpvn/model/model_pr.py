...
class ConformerTransformerModel(pl.LightningModule):
    ...
    def test_step(self, batch: tuple, batch_idx: int) -> Tensor:
        ...
        y_hats = outputs.max(-1)[1]
        per = self.per_metric(targets[:, 1:-1], y_hats[:, :-1])
        ...
        with open('test.result', 'a') as file_result:
            print(
                utt_id,
                per,
                self.vocab.label_to_string(y_hats[0, :-1]).replace('   ', '-').replace(' ', ''),
                self.vocab.label_to_string(targets[0, 1:-1]).replace('   ', '-').replace(' ', ''),
                sep=',' ,
                file=file_result
            )
...
class ConformerARNNModel(pl.LightningModule):
    def __init__(
            self,
            configs: DictConfig,
            num_classes: int,
        ...
        super(ConformerARNNModel, self).__init__()
        self.configs = configs
        self.gradient_clip_val = configs.gradient_clip_val
        self.teacher_forcing_ratio = configs.teacher_forcing_ratio
        self.vocab = vocab
